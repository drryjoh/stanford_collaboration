...
for index, row in openfoam_data.iterrows():
    ...
    x_front = sum(
        ...
        np.sin(2 * np.pi * rng.uniform(1, 20) * y_cell / 0.06 + rng.uniform(0, 2*np.pi))
        for _ in range(20)
    ) + 0.05
    z_front = sum(
        0.001 * rng.uniform(0.5, 1.5) *
        np.sin(2 * np.pi * rng.uniform(1, 20) * x_cell / 0.06 + rng.uniform(0, 2*np.pi)) *
        np.sin(2 * np.pi * rng.uniform(1, 20) * z_cell / 0.06 + rng.uniform(0, 2*np.pi))
        for _ in range(20)
    ) + 0.05

    interpolated_row = np.array([])
    if x_cell < x_front and z_cell < z_front:
